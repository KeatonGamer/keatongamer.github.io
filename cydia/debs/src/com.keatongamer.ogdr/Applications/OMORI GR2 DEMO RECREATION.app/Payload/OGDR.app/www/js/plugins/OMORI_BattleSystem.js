...
//Why do I have to go through this again? Why didn't I document it like a normal human being?

// Archeia Changes.
// Remove the fucking party command window. I hate this.
...
Scene_Boot.loadSystemImages = function() {
  ...
  // ImageManager.reserveSystem('ACS_Bubble')
    ImageManager.reserveSystem('bsys_commandwindow_dream_attack');
    ImageManager.reserveSystem('bsys_commandwindow_dream_calmdown');
    ImageManager.reserveSystem('bsys_commandwindow_dream_donothing');
    ImageManager.reserveSystem('bsys_commandwindow_dream_pocket');
    ImageManager.reserveSystem('bsys_commandwindow_dream_run');
    ImageManager.reserveSystem('bsys_commandwindow_dream_skill');
    ImageManager.reserveSystem('target_selected');
...
BattleManager.displayVictoryMessage = function () {
    Scene_Battle.prototype.hideFaceWindows();
    //Scene_Battle.prototype._faceWindowsContainer.visible = false;
//    $gameMessage.addText(TextManager.victory.format($gameParty.name()))
...
// $gameMessage.addText(TextManager.defeat.format($gameParty.name()));
...
Scene_Battle.prototype.update = function() {
  // Run Original Function
    ...
    switch (this._actorCommandWindow.currentSymbol()) {
        ...
        //=======
        case 'actionSkill':
            switch (this._actorCommandWindow.currentExt()) {
              case 1:
                  //console.log('attack');
                    this._actorCommandWindow.changeCommandSprites(0);
                  break;
              case 1391:
                  //console.log('AubreyDoNothing');
                    this._actorCommandWindow.changeCommandSprites(2); 
                  break;
              case 1392:
                  //console.log('HeroDoNothing');
                    this._actorCommandWindow.changeCommandSprites(2); 
                  break;
              case 1393:
                  //console.log('KelDoNothing');
                    this._actorCommandWindow.changeCommandSprites(2); 
                  break;
              case 1394:
                  //console.log('OmoriDoNothing');
                    this._actorCommandWindow.changeCommandSprites(2); 
                  break;
              case 1390:
                  //console.log('calmdown');
                    this._actorCommandWindow.changeCommandSprites(1); 
                  break;
              case 6:
                  //console.log('runaway');
                    this._actorCommandWindow.changeCommandSprites(4); 
                  break;
        };
        ...
        case 'guard':
            //console.log('pocket');
            this._actorCommandWindow.changeCommandSprites(3);
            break;
        ...
        case 'skill':
            //console.log('skill');
            this._actorCommandWindow.changeCommandSprites(5);
            break;
...
Scene_Battle.prototype.createAllWindows = function() {
  ...
  this._partyCommandWindow.x = 30
  this._partyCommandWindow.y = 15
  this._actorCommandWindow.x = 30
  this._actorCommandWindow.y = 15
...
Scene_Battle.prototype.createHelpWindow = function() {
  // Create Help Window
  this._helpWindow = new Window_OmoMenuHelp(190, 102);
...
Scene_Battle.prototype.createSkillWindow = function() {
  ...
  this._skillWindow = new Window_BattleSkill(140, 15, 345, 96);
...
Scene_Battle.prototype.createItemWindow = function() {
  this._itemWindow = new Window_BattleItem(140, 15, 345, 96);
...
Scene_Battle.prototype.onActorOk = function() {
  // Get Action
    console.log(this._actorCommandWindow.customCursorRectYOffset());
  let action = BattleManager.inputtingAction();
  // Get Target
...
Scene_Battle.prototype.createFaceWindows = function() {
  ...
  // Create Layer List
  var layerList = ['behind', 'statusBack', 'polaroid', 'face', 'front']
  // Create Layers
  ...
  for (var i = 0; i < $gameParty.size(); i++) {
    ...
    var index = i;
      ...
      else if ($gameParty.members().length == 3) {
          var x = 111 + (i * (155 - 23));

      }
      else if ($gameParty.members().length == 2) {
          var x = 178 + (i * (155 - 23));
  ...
  // Set status Windows Face Windows
    ...
    this._faceWindowsContainer.visible = false;
};
//=============================================================================
// * Hide face windows
//=============================================================================
Scene_Battle.prototype.hideFaceWindows = function () {
    console.log("hidethem");
    SceneManager._scene._faceWindowsContainer.visible = false;
    SceneManager._scene._faceWindowsContainer.x = 1000;
};
//=============================================================================
// * Create Face Windows
//=============================================================================
Scene_Battle.prototype.createStressBar = function() {
  ...
  this._stressBar.y = Graphics.height - 999;
...
Scene_Battle.prototype.startPartyCommandSelection = function () {
    this.commandFight();
    this._faceWindowsContainer.visible = true;
...
Scene_Battle.prototype.showItemWindow = function() {
  ...
  // Create Movement Data
  var data = {
    obj: obj, properties: ['y'],  from: {y: obj.y}, to: {y: obj.y}, durations: {y: duration},
    easing: Object_Movement.easeOutCirc
  };
  this.move.startMove(data);
...
Window_OmoriBattleActorStatus.prototype.windowWidth = function() { return 119; };
...
Window_OmoriBattleActorStatus.prototype.createSprites = function() {
  ...
  this._hpBarSprite.x = pos.x + 0
  this._hpBarSprite.y = pos.y + 0;
  this._hpBarSprite.setFrame(0, 0, 102, 6);
  ...
  this._mpBarSprite.setFrame(0, 19, 102, 6);
...
Window_OmoriBattleActorStatus.prototype.drawHP = function(hp, maxHP) {
  ...
  this.contents.clearRect(0, 0, this.width, 70);
  var maxText = ' ' + maxHP
  var width = this.textWidth(maxText) + 11;
  this.changeTextColor(this.textColor(6));
  this.drawText(hp, 7, y, this.width - width, 'right');
  this.drawText(maxText, 7, y, this.width - 10,'right');
  this.drawText('/', 7, y + 1, this.width - width + 5, 'right');

  // Draw Name
  this.contents.fontSize = 20;
  this.contents.fontSize = LanguageManager.getMessageData("XX_BLUE.Window_ActorBattleName").drawHP_contents_fontsize;
  this.resetFontSettings();
  this.changeTextColor(this.textColor(6));
  var actor = this.actor();
  if (actor) {
     this.drawText(actor.name(), 45, 1, this.width - 11, 'left');
  };
};
//=============================================================================
// * Draw HP
...
Window_OmoriBattleActorStatus.prototype.drawMP = function(mp, maxMP) {
  ...
  var width = this.textWidth(maxText) + 11;
  this.changeTextColor(this.textColor(6));
  this.drawText(mp, 4, y, this.width - width, 'right');
  this.drawText(maxText, 4, y, this.width - 10,'right');
  this.drawText('/', 4, y + 1, this.width - width + 5,'right');
  this.resetFontSettings()
};
//=============================================================================
// * Frame Update
...
Window_OmoriBattleActorStatus.prototype.updatePositions = function() {
  ...
  this._faceSprite.x = this.x + ((this.width - 106) / 2)+2;
  this._faceSprite.y = this.y + 4

  this._hpBarSprite.x = this.x + 14
  this._hpBarSprite.y = this.y + 65;

  this._mpBarSprite.x = this.x + 14
  this._mpBarSprite.y = this.y + 88;

  this._stateSprite.x = this.x + this.width / 2;
  this._stateSprite.y = this.y + 14;

  this._selectedOverlay.x = this.x + -12
  this._selectedOverlay.y = this.y + -5
...
// * Update Shake
//=============================================================================
Window_OmoriBattleActorStatus.prototype.updateShake = function () {
    // If Shake is true
    if (this._shake) {
        // Get Shake Power
        var power = 2
        if (Math.random() >= 0.5) {
            this.x = this.xpos + power;
        } else {
            this.x = this.xpos - power;
        };
        if (Math.random() >= 0.5) {
            this.y = this.ypos + power;
        } else {
            this.y = this.ypos - power;
        };
...
// * Start Shake
//=============================================================================
Window_OmoriBattleActorStatus.startShake = function () {
...
// * Stop Shake
//=============================================================================
Window_OmoriBattleActorStatus.stopShake = function () {
    // Set Shake Flag to true
    this._shake = false;
    // Reset Anchor
    this.anchor.set(0, 0);
    ...
    if (actor.isDamagePopupRequested()) {
      ...
      if (result.hpDamage > 0) {
        // Set Damage Face Animation
          this._shake = true;
        if(!$gameTemp._secondChance || actor.actorId() !== 1) {}
      };
      this._popupCount++;
      // Clear Results
  ...
  for (var i = 0; i < this._damageSprites.length; i++) {
    ...
    // If Damage is not playing anymore
      ...
      if (!sprite.isPlaying()) {
          ...
          this.x = this.xpos
          this.y = this.ypos
          this._removeDamageSprites.push(sprite);
      };
  };
...
Window_OmoriBattleActorStatus.prototype.updateBars = function() {
  ...
  if (this._hpAnim.duration > 0) {
    ...
    var width = (anim.current / actor.mhp) * 102;
  ...
  if (this._mpAnim.duration > 0) {
    ...
    var width = (anim.current / actor.mmp) * 102;
...
Window_OmoriBattleActorStatus.prototype.updateOverlay = function() {
  ...
  if (this._selected) {
    ...
    if (anim.delay <= 0) {
      if (anim.side === 0) {
        this._selectedOverlay.opacity += 255;
        if (this._selectedOverlay.opacity >= 255) {
          anim.side = 1; anim.delay = 1;
        };
      } else {
        this._selectedOverlay.opacity -= 0;
        if (this._selectedOverlay.opacity <= 0) {
          anim.side = 0; anim.delay = 1;
        };
      };
    } else {
      // Decrease Delay
...
Window_ActorCommand.prototype.initialize = function() {
  // Set Custom Cursor X Offset
    ...
    this._customCursorYOffset = 5;
  // Set Max Command Columns
  this._commandMaxCols = 3;
  // Super Call
...
// * Settings
//=============================================================================
Window_ActorCommand.prototype.windowWidth = function() { return 352; };
...
Window_ActorCommand.prototype.itemWidth = function() { return 104; };
Window_ActorCommand.prototype.customCursorRectXOffset = function() { return this._customCursorXOffset; }
Window_ActorCommand.prototype.customCursorRectYOffset = function () { return 999; }
...
Window_ActorCommand.prototype.createCommandSprites = function () {
    ...
    var x = 142;
    var y = Graphics.boxHeight - 156;
    // Position Type 0: Outside The screen, 1: ECG Visible, 2: Fully visible
    ...
    this.refresh();
    // Create Background Sprite
    if ($gameVariables.value(2000).toString() == "Window_DreamWorld") {
        this._backgroundSprite = new Sprite(ImageManager.loadSystem('bsys_commandwindow_dream'));
    } else {
        if ($gameVariables.value(2000).toString() == "Window_Faraway") {
            this._backgroundSprite = new Sprite(ImageManager.loadSystem('bsys_commandwindow_faraway'));
        } else {
            this._backgroundSprite = new Sprite(ImageManager.loadSystem('bsys_commandwindow'));
        }
    }

    this.addChildToBack(this._backgroundSprite);
    // Create Static Bar Sprite


};

Window_ActorCommand.prototype.changeCommandSprites = function (SpriteNumber) {
    if (SpriteNumber == 0) {
        this._backgroundSprite.bitmap = ImageManager.loadSystem('bsys_commandwindow_dream_attack');
        //console.log('attackIMAGE');
        
    }
    else if (SpriteNumber == 1) {
        this._backgroundSprite.bitmap = ImageManager.loadSystem('bsys_commandwindow_dream_calmdown');
        //console.log('calmIMAGE');
        
    }
    else if (SpriteNumber == 2) {
        this._backgroundSprite.bitmap = ImageManager.loadSystem('bsys_commandwindow_dream_donothing');
        //console.log('nothingIMAGE');
        
    }
    else if (SpriteNumber == 3) {
        this._backgroundSprite.bitmap = ImageManager.loadSystem('bsys_commandwindow_dream_pocket');
        //console.log('pocketIMAGE');
        
    }
    else if (SpriteNumber == 4) {
        this._backgroundSprite.bitmap = ImageManager.loadSystem('bsys_commandwindow_dream_run');
        //console.log('runIMAGE');
        
    }
    else if (SpriteNumber == 5) {
        this._backgroundSprite.bitmap = ImageManager.loadSystem('bsys_commandwindow_dream_skill');
...
// * Settings
//=============================================================================
Window_ItemListBack.prototype.standardPadding = function() { return 0 };
...
Window_ItemListBack.prototype.setItem = function(item) {
  // Clear Rect
  this.contents.clearRect(0, 0, this.contents.width, 10);
  this._backgroundSprite = new Sprite(ImageManager.loadSystem('itemwindow'));
  this.addChildToBack(this._backgroundSprite);
  //this.changeTextColor(this.textColor(6));
  ...
  //  this.contents.bitmapFontColor = null;
      ...
      //this.contents.drawText(LanguageManager.getMessageData("XX_BLUE.Omori_Battle_System").cost + this._actor.skillMpCost(item), 6, 2, 100, 20);
...
Window_BattleItem.prototype.initialize = function(x, y, width, height) {
  ...
  // Super Call
  Window_ItemList.prototype.initialize.call(this, x, y, width, 90);
  // Create Back Window
  this._backWindow = new Window_ItemListBack(width+30, height)
...
Window_BattleItem.prototype.lineHeight = function() { return 23; };
Window_BattleItem.prototype.spacing = function() { return 0; };
Window_BattleItem.prototype.standardPadding = function() { return 10; };
Window_BattleItem.prototype.customCursorRectXOffset = function () { return -7; }
Window_BattleItem.prototype.maxCols = function () { return 1 };
...
Window_BattleSkill.prototype.initialize = function(x, y, width, height) {
  // Super Call
  Window_SkillList.prototype.initialize.call(this, x, y, width, 90);
  // Create Back Window
...
Window_BattleSkill.prototype.lineHeight = function() { return 23; };
Window_BattleSkill.prototype.spacing = function() { return 0; };
Window_BattleSkill.prototype.standardPadding = function() { return 10; };
Window_BattleSkill.prototype.customCursorRectXOffset = function () { return 212; }
Window_BattleSkill.prototype.maxCols = function () { return 1 };
...
Window_ScrollingTextSource.prototype.drawTextEx = function(text, x, y) {
  ...
  this.contents.bitmapFontColor = null;
  if (text) {
    var textState = { index: 0, x: x+27, y: y+8, left: x };
...
Window_BattleLog.prototype.initialize = function() {
  ...
  //this._logWindowSprite = new Sprite(ImageManager.loadSystem('bsys_battlelog'));
  //this._logWindowSprite.x = 30;
  //this._logWindowSprite.y = 10;
  //layers._behind.addChild(this._logWindowSprite);


    this.spacing = function () { return 27; };

  this.contents.clearRect(0, 0, this.contents.width, 28);
  this._backgroundSprite = new Sprite(ImageManager.loadSystem('bsys_battlelog'));
  this.addChildToBack(this._backgroundSprite);
...
// * Settings
//=============================================================================
Window_BattleLog.prototype.windowWidth = function() { return 578; };
Window_BattleLog.prototype.windowHeight = function() { return 91 };
//=============================================================================
    ...
    if (this._actor.hp !== this._oldHp) {
      ...
      if (!this._hidden) {
        ...
        if (this._actor.hp === 0) {
          ...
          // Play Low HP Background Sound
            AudioManager.playDangerBgs({name: 'boss_something_heartbeat', volume: 0, pitch: 1 })
            // Set Volume
